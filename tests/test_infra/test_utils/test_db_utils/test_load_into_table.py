...
Key behaviors
-------------
- Confirms that `flush_values_batch` is never called for an empty iterator.
...
Conventions
-----------
- Uses `mocker.spy` to observe calls into `flush_values_batch`.
...
def test_load_into_table_empty_iter(mocker: MockerFixture) -> None:
    ...
    mocker : MockerFixture
        Pytest-mock fixture used to spy on `flush_values_batch`.

    Raises
    ------
    AssertionError
        If `flush_values_batch` is called despite an empty iterator.
    """

    spy_flush_values_batch: MagicMock = mocker.spy(dbu, "flush_values_batch")
    ...
    spy_flush_values_batch.assert_not_called()
...
def test_load_into_table_row_generator(
    ...
    `flush_values_batch` the expected number of times across different batch sizes.
    ...
    # Assertions
    assert spy_flush_values_batch.call_count == ceil(row_amount / batch_size)
    expected_last_batch_size = (row_amount % batch_size) or batch_size
    for i in range(spy_flush_values_batch.call_count):
        args, _ = spy_flush_values_batch.call_args_list[i]
        assert args[0] is mock_conn
        batch_arg: List[tuple[int]] = args[1]
        assert args[2] == input_query
        if i < spy_flush_values_batch.call_count - 1:
