...
--
-- Purpose
--   Minimal, durable ledger of (ticker, validity_window) pairs that have
--   fully completed EDGAR evidence extraction, so later runs can safely
--   skip work already persisted.
--
-- Row semantics
--   One row = one completed extraction episode for a specific
--   (ticker, validity_window). A row is inserted only after the evidence
--   for that pair is successfully written (atomic with the evidence load).
--
-- Conventions
...
--
-- Keys & constraints
--   - Primary key: (ticker, validity_window) to enforce one completion record
--     per canonical window across all runs.
--   - Checks: ticker format; validity_window shape; end_time >= start_time.
...
--     (ticker, validity_window) pairs to skip.
--
-- Audit & provenance
...
--     WHERE ticker = ? AND validity_window = ?.
--   - The EXCLUDE constraint (if enabled)
--     creates a GiST index for overlap checks.
--
-- Change management
--   - Add-only compatible: new columns
--     may be added with defaults or NULLability
--     without breaking consumers; avoid changing primary key semantics.
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS edgar_run_registry (
    ...
    CONSTRAINT edgar_run_registry_pk PRIMARY KEY (ticker, validity_window),
    ...
    -- Prevent overlapping extraction windows for the same ticker
    CONSTRAINT ex_no_overlap_per_ticker
    EXCLUDE USING gist (
        ticker WITH =,
...
'Prevents overlapping validity windows per ticker (GiST with btree_gist).';
