...
--     confidence label/score consistency; filed_at ∈ validity_window.
--
-- Relationships
...
CREATE TABLE IF NOT EXISTS ticker_cik_mapping (
    ...
    -- (e.g., edgar_fts, submissions_verify, company_tickers, manual_backfill).
    source TEXT NOT NULL,

    -- EDGAR accession number for exact traceability;
    accession_num TEXT NOT NULL,

    -- Qualitative confidence label for the mapping (e.g., high, medium)
    -- derived from confidence_score (high >= 0.8, 0.3 <= med < 0.8, low < 0.3).
    confidence TEXT NOT NULL,

    -- Qualitative confidence score for the mapping (0.0 - 1.0).
    ...
    CONSTRAINT ck_confidence_value
    CHECK (confidence IN ('high', 'medium', 'low')),

    -- Confidence score guard
    CONSTRAINT ck_confidence_score_range
    CHECK (confidence_score >= 0.0 AND confidence_score <= 1.0),

    -- Enforce consistency between confidence and confidence_score
    CONSTRAINT ck_confidence_score_consistency
    CHECK (
        (confidence = 'high' AND confidence_score >= 0.8)
        OR
        (
            confidence = 'medium'
        ...
        OR
        (confidence = 'low' AND confidence_score < 0.3)
    ),
...
COMMENT ON COLUMN ticker_cik_mapping.confidence IS
'Qualitative label (high/medium/low) consistent with confidence_score policy.';

COMMENT ON COLUMN ticker_cik_mapping.confidence_score IS
'Calibrated score in [0,1] used for selection and labeling.';
...
COMMENT ON CONSTRAINT ex_no_overlap_per_ticker ON ticker_cik_mapping IS
